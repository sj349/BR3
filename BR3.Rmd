...
a. Beta(5, 5) because this represents a bell curve with the mean centered around 0.5. Since the friend first estimated 0.4, and then shifted their estimate to anywhere in the 0.2 to 0.6 range, we can assume that a bell curve centered around 0.5 is a good representation of this probability. 
```{r}
plot_beta(5, 5)
```

b. Beta(5, 2) because this represents a right-skewed model. Since the mean estimate is 0.8 with a low variance (0.05), this right skewed model with a peak towards pi==0.8 seems to be the best fit. 
...
c. Beta(5, 1.5) because this represents a right-skewed model. Since the mean estimate is 0.9 with a range between 0.85 and 1, this right skewed model with a peak towards pi==0.9 seems to be the best fit. 
```{r}
plot_beta(5, 1.5)
```
...
a. Beta(5, 2) because this represents a right-skewed model. Since the mean estimate is 0.8 with a range between 0.7 and 0.9, this right skewed model with a peak towards pi==0.8 seems to be the best fit. 
```{r}
plot_beta(5, 2)
```

b. Beta(5, 1.5) because this represents a right-skewed model. Since the mean estimate is 0.9 with a range between 0.85 and 1, this right skewed model with a peak towards pi==0.9 seems to be the best fit. 
```{r}
plot_beta(5, 1.5)
```

c.  Beta(5, 1.57) because this represents a right-skewed model. Since the mean estimate is 0.85 with a range between 0.75 and 0.95, this right skewed model with a peak towards pi==0.85 seems to be the best fit.  
```{r}
plot_beta(5, 1.57)
```

d. Beta(2.85,5) because this represents a slightly left-skewed model. Since the mean estimate is 0.3 with a high level of variance, this left skewed model with a peak towards pi==0.3 seems to be the best fit.  
```{r}
plot_beta(2.85, 5)
```
...
b. The mean is 0.5. This aligns with having no clue about f(pi) because if you have no idea whether f(pi) is skewed more towards 1 or 0, its most likely going to be in the middle.
